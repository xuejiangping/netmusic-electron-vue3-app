...
const formatedData = computed(() => {
    return props.dataList?.map(({ id, creator, name, userId, playCount, trackCount, coverImgUrl, bookCount }) => ({
        name, userId, trackCount, coverImgUrl, bookCount, playCount,
        creator: { id: creator.userId, name: creator.nickname }, id
    }))
})
</script>

<template>
    <router-link v-for="({ name, trackCount, creator, id, playCount, coverImgUrl }) in formatedData"
